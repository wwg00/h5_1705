			...
			var lx = {
				init:function(){
					...
					if(opt.buttons){
						...
						var $btnprev=$('<span/>').addClass('btn-prev btn').appendTo($self).addClass('glyphicon glyphicon-chevron-left');
						var $btnnext=$('<span/>').addClass('btn-next btn').appendTo($self).addClass('glyphicon glyphicon-chevron-right');
						//btn类用来隐藏;
						//用了bootstrap组件,字体图标;
					}
					...
					$self.hover(function(){
                        ...
                        //移入时显示;
                       $btnprev.removeClass('btn');
                       $btnnext.removeClass('btn');
					},function(){
						 $btnprev.addClass('btn');
					...
					//利用事件委托实现点击按钮切换图片
                    this.$page.on('mouseover','span',function(e){
				...
				show:function(){
					...
					if(opt.type === 'seamless'){
						...
						this.$ul.find('li').eq(opt.index).fadeIn(200).siblings().fadeOut(200);
						this.$page.children('span').removeClass('active');
						this.$page.children('span').eq(opt.index).addClass('active');
					}
